...
def eval_metrics_query_and_gallery_dataset(model, query_dataloader, gallery_dataloader, device, k_vals, opt):
    ...
    with torch.no_grad():
        ...
        query_target_labels, query_feature_coll = np.hstack(query_target_labels).reshape(-1, 1), np.vstack(
            query_feature_coll).astype('float32')
        gallery_target_labels, gallery_feature_coll = np.hstack(gallery_target_labels).reshape(-1, 1), np.vstack(
            gallery_feature_coll).astype('float32')
        ...
        _, k_closest_points = faiss_search_index.search(query_feature_coll, int(np.max(k_vals) + 1))
    ...
    def update_info_plot(self):
        ...
        if self.prop.dataset != 'vehicle_id':
            ...
            for i in range(3):
                title = ' | '.join(
                    key + ': {0:3.3f}'.format(np.max(item)) for key, item in self.progress_saver['val'].items() if
                    key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key)
                self.info_plot['Set {}'.format(i)].title = title
                v_metric_list = [self.progress_saver['val'][key] for key in self.progress_saver['val'].keys() if
                                 key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key]
                v_legend_handles = [key for key in self.progress_saver['val'].keys() if
                                    key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key]
                self.info_plot['Set {}'.format(i)].make_plot(t_epochs, v_epochs, t_loss_list, v_metric_list,
                                                             t_legend_handles, v_legend_handles,
