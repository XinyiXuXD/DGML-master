...
def Give(opt, datapath):
    ...
    if not os.path.exists(image_sourcepath):
        pro(datapath)
    image_classes = sorted([x for x in os.listdir(image_sourcepath)])
    total_conversion = {i: x for i, x in enumerate(image_classes)}
    image_list = {i: sorted([image_sourcepath + '/' + key + '/' + x for x in os.listdir(image_sourcepath + '/' + key)])
...
def pro(datapath):
    ...
    cars_path = datapath + '/cars_annos.mat'
    cars = mat4py.loadmat(cars_path)

    labels = np.array(cars['annotations']['class'], dtype=np.int32)
    relative_im_path = np.array(cars['annotations']['relative_im_path'])
    uni_labels = np.unique(labels)
    count = 0
    for i in range(len(uni_labels)):
        this_class = uni_labels[i]
        this_folder_name = str(this_class).rjust(3, '0') + '.' + cars['class_names'][i].replace('/', '')
        target_dir = datapath + '/images' + '/' + this_folder_name
        os.mkdir(target_dir)
        this_class_relative_im_path = relative_im_path[labels == this_class]
        for j, p in enumerate(this_class_relative_im_path):
            if count % 1000==0:
                print('processing the {}th img'.format(count))
            count += 1
            source_im = datapath + '/' + p
            target_im = target_dir + '/' + str(count).rjust(6, '0') + '.jpg'
            copyfile(source_im, target_im)