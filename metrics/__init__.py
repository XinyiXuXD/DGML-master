...
class MetricComputer():
    ...
    def computer_metrics(self, opt, feature_colls, target_labels, evaltypes, dataloader):
        ...
        for evaltype in evaltypes:
            ...
            for metric in self.list_of_metrics:
                ...
                if 'kmeans_nearest' in metric.requires:   input_dict[
    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, **kwargs):
        ...
        with torch.no_grad():
            ...
            for idx, inp in enumerate(final_iter):
                ...
                if opt.att_vis and opt.group_name == 'learn_q':
                    att_wei.append(aux_f.cpu().detach().numpy())
                    imgs.append(input_img)
                if opt.att_vis and opt.group_name == 'gate':
                    att_wei.append(aux_f.cpu().detach().numpy())
                    imgs.append(input_img)
                elif opt.att_vis and opt.group_name == 'channel':
                    att_wei.append(aux_f.cpu().detach().numpy())
                    imgs.append(input_img)
                    activation = {}
                    ...
                    model.module.backbone[-1][-1].conv3.register_forward_hook(get_activation('conv3'))
                    out = model(input_img.to(device))
                    fp = activation['conv3'].cpu().detach().numpy()
                    channels.append(fp)
            ...
            if opt.att_vis:
                att_wei, imgs = np.concatenate(att_wei, axis=0), np.concatenate(imgs, axis=0)
                if opt.group_name == 'channel':
        ...
        if opt.att_vis:
            if opt.group_name == 'channel':
                return computed_metrics, extra_infos, feature_colls, target_labels, att_wei, imgs, channels
            else:
                return computed_metrics, extra_infos, feature_colls, target_labels, att_wei, imgs
        else:
            return computed_metrics, extra_infos, feature_colls, target_labels
